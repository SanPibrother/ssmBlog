		...
		var info = {
			...
			tip : function() {
				$("a, :button, span").each(function() {
					...
					if (tip != null) {
						var html = '<div id="' + tip + '" class="javaex-tip" style="position: absolute;z-index: 9999999; display: none;">';
						html += '<div class="javaex-tip-arrow"></div>';
						html += '<div class="javaex-tip-inner">' + tip + '</div>';
						html += '</div>';$(this).append(html);$(this).css({
							"position" : "relative",
							"overflow" : "inherit"
			...
			comment : function(args) {
				...
				for (var i = 0; i < list.length; i++) {
					...
					html += '<span style="color:#959595;">' + list[i][commentMapping.time] + '</span>';var replyList = list[i][commentMapping.replyInfoList];
					if (userId == "" || userId == null) {
					} else {
				...
				html += '</ul>';$("#" + id).empty();$("#" + id).append(html)
			},
			javaexComment : function(obj, args) {
				var opt = defaults(args);
				var content = $(obj).parent().parent().find("textarea").val().replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "").replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "");
			...
			javaexShowReplyTextarea1 : function(obj, args) {
				...
				if (posrLength == 0) {
					...
					html += '</div>';
					html += '</div>';$(obj).parent().after(html)
				} else {
			...
			tag : function(args) {
				...
				if (tagArr != "" && tagArr != null) {
					...
					$("#" + id).val(tag);$(".tags").append(tagHtml)
				}
			},
			saveTag : function(tag, id, maxNum) {
				...
				html += '</div>';$(".tag-input").val("");$(".tags").append(html)
			},
			...
			filter : function(args) {
				...
				callback();$(selector + " li").click(function() {
			...
			score : function(args) {
				...
				liHtml += '<strong class="level-text"></strong>';$("#" + id).append(liHtml);
				scoreChange = function(elem, index) {
					return $(elem).each(function(i, scoreElem) {
						return $(scoreElem).find("i").each(function(i, item) {
							if (i <= index) {
								$(item).removeClass("icon-star-o");return $(item).addClass("icon-star")
							} else {
								$(item).removeClass("icon-star");return $(item).addClass("icon-star-o")
							}
						})
					})
				};
				...
				if (score != "") {
					$("#" + id + " li").each(function() {
						if ($(this).attr("score") == score) {
							...
							index--;$(this).parent().find(".level-text").html(opt.levelTextArr[index]);scoreChange($("#" + id), index);return false
						}
					})
				}
				if (!isReadOnly) {
					$("#" + id + " li").hover(function() {
						...
						index--;$(this).parent().find(".level-text").html(opt.levelTextArr[index]);return scoreChange($(this).parent(), index)
						...
						$("#" + id).attr("active", index);scoreChange($("#" + id), index - 1);opt.callback({
							"index" : index,
			...
			alert : function(args) {
				...
				alertHtml += '</div>';
				alertHtml += '</div>';
				alertHtml += '</div>';$(document.body).append(alertHtml);
			...
			confirm : function(args) {
				...
				confirmHtml += '</div>';
				confirmHtml += '</div>';
				confirmHtml += '</div>';$(document.body).append(confirmHtml);
				var oDialog = document.getElementById(UUID);
				var oDrag = document.getElementById("dialog-top-" + UUID);
				info.drag(oDialog, oDrag)
			},
			deleteDialog : function(obj, args) {
				...
				followHtml += '</div>';
				followHtml += '</div>';
				followHtml += '</div>';$(document.body).append(followHtml)
			},
			followDialog : function(obj, args) {
				...
				followHtml += '</div>';$(document.body).append(followHtml);
				if (position == "right") {
					$(".panel").css("top", (top + obj.offsetHeight / 2 - $(".panel").height() / 2) + "px");$('head').append('<style>.panel.arrow-left:before{left:-8px;top:' + ($(".panel").height() / 2 - 9) + 'px;}</style>')
				}
			...
			optTip : function(args) {
				...
				if (type == "submit") {
					...
					if ($(".tip-icon-loading").length > 0) {
						$(".opt-tip").css("background-color", "#79c37b");$(".opt-tip .tip-icon").removeClass("tip-icon-loading");$(".opt-tip .tip-icon").addClass("icon-check");$(".opt-tip .tip-icon").css({
							"font-size" : "16px",
							"font-weight" : "bold"
					...
					if ($(".tip-icon-loading").length > 0) {
						$(".opt-tip").css("background-color", "#ff6e6e");$(".opt-tip .tip-icon").removeClass("tip-icon-loading");$(".opt-tip .tip-icon").addClass("icon-close");$(".opt-tip .tip-icon").css({
							"font-size" : "16px",
							"font-weight" : "bold"
						});$(".opt-tip .tip-msg").text(opt.content)
					} else {
				...
				if (type == "success" || type == "error") {
					setTimeout(function() {
						$(".opt-tip").removeClass("fadeInDown");$(".opt-tip").addClass("zoomOut");$(".opt-tip").remove();$(".mask").remove()
					}, opt.live)
				}
			},
			...
			dialog : function(args) {
				...
				if (type == "login") {
					...
					var heightDifference = document.documentElement.clientHeight - oDialog.offsetHeight;
					oDialog.style.top = (heightDifference / 2) + "px";$(".mask").click(function() {
						info.close(UUID)
					})
				} else {
			...
			resize : function(UUID, args) {
				...
				oResize.onmousedown = function(event) {
					...
					document.onmousemove = function(eve) {
						...
						document.getElementById("dialog-content-" + UUID).style.height = (offsetHeight - 80) + "px";return false
					};
			...
			close : function(UUID) {
				if (!UUID) {
					$(".dialog").removeClass("zoomIn");$(".dialog").addClass("zoomOut");$(".mask").remove();setTimeout(function() {
						$(".dialog").remove()
					}, 300)
				} else {
					var oDialog = document.getElementById(UUID);
					if (oDialog.getAttribute("class") == "danger-dialog") {
						...
						oDialog.classList.remove("zoomIn");oDialog.classList.add("zoomOut");$(".mask").remove();setTimeout(function() {
							oDialog.remove()
						}, 300)
					}
				}
				...
				$("script").each(function() {
					if ($(this).attr("src") == "" || $(this).attr("src") == null || $(this).attr("src") == undefined) {
						...
						if ($(this).attr("src").indexOf("common.js") > -1) {
							...
							script.src = jsSrc;$(document.body).append(script)
						}
			...
			nav : function(args) {
				...
				function navSelected(url) {
					$("#" + navId + " li>a").each(function() {
						...
						if (url.indexOf(href) >= 0) {
							flag = true;setCookie("navUrl", url);$(this).parent("li").addClass("active").siblings("li").removeClass("active")
						}
					})
				}
				...
				function delCookie(key) {
					var exp = new Date();
					exp.setTime(exp.getTime() - 1);var cval = getCookie(key);
					if (cval != null) {
			...
			iframeMenu : function(args) {
				$(".admin-iframe-content ul.menu>li>a").click(function() {
					$(".admin-iframe-content ul.menu>li.active").removeClass("active");$(this).parent("li").addClass("active")
				})
			},
			menu : function(args) {
				var opt = defaults(args);
				...
				$("#" + menuId + " .menu-item>a").on("click", function() {
					if ($(this).next().css("display") == "none") {
						$(".menu-item").children("ul").slideUp(300);$(this).next("ul").slideDown(300);$(this).parent("li").addClass("menu-show").siblings("li").removeClass("menu-show")
					} else {
				...
				function addHover(obj) {
					$("#" + menuId + " li.menu-item").removeClass("hover");$("#" + menuId + " li.menu-item ul li").removeClass("hover");obj.addClass("hover")
				}
				if (opt.isAutoSelected) {
					var flag = false;
					var url = window.location.href;
				...
				function menuSelected(url) {
					$("#" + menuId + " li").each(function() {
						...
						if (url.indexOf(href) >= 0) {
							flag = true;setCookie("adminMenuUrl", url);
							if ($(this).hasClass("menu-item")) {
								$("#menu li.menu-item").removeClass("hover");$("#menu li.menu-item ul li").removeClass("hover");$(this).addClass("hover")
							} else {
								$(this).addClass("hover").siblings().removeClass("hover");$(this).parent().parent().addClass("menu-show")
							}
						}
					})
				}
				...
				if (mode == "mouseover") {
					$("#" + tabId + " .tab-title ul li").mouseover(function() {
						$this = $(this);setTimeout(function() {
							$this.addClass("current").siblings().removeClass("current");$("#" + tabId + " .tab-content>div:eq(" + $this.index() + ")").show().siblings().hide();opt.callback({
								"index" : $this.index() + 1
							})
						}, opt.delay)
					})
			...
			table : function(args) {
				...
				function tableMerge(obj, colIndex) {
					obj.data("content", "");obj.data("rowspan", 1);obj.data("td", $());obj.data("trNum", $("tbody tr", obj).length);$("tbody tr", obj).each(function(index) {
						var $tr = $(this);
						var $td = $("td:eq(" + colIndex + ")", $tr);
						var currentContent = $td.html();
						if (obj.data("content") == "") {
							obj.data("content", currentContent);obj.data("td", $td)
						} else {
						...
						function addRowspan() {
							...
							obj.data("rowspan", rowspan);$td.hide();
						...
						function newRowspan() {
							...
							obj.data("td", $td);obj.data("content", $td.html());obj.data("rowspan", 1)
						}
					})
				}
			},
			select : function(args) {
				var opt = defaults(args);
				var selectId = opt.id;
				$("#span-" + selectId).remove();$("#" + selectId).css("display", "none");
				...
				selectHtml += '</ul>';
				selectHtml += '</span>';$("#" + selectId).before(selectHtml);
				...
				$("#input-" + selectId).css("width", width + "px");$("#ul-" + selectId).empty();$("#ul-" + selectId).html($("#" + selectId).html());$("#ul-" + selectId + " option").addClass("select-ul-item");$("#ul-" + selectId + " option").each(function() {
					if ($(this).val() == $("#" + selectId).val()) {
						$(this).addClass("select-ul-item-selected");$("#input-" + selectId).val($(this).text());return false
					}
				});$("#ul-" + selectId).css("max-height", 28 * parseInt(opt.maxNum) + 2 + "px");$("#span-" + selectId).click(function(event) {
					if ($("#" + selectId).prop("disabled")) {
						event.preventDefault()
					} else {
						$("#ul-" + selectId + " option").removeClass("select-ul-item-selected");$("#ul-" + selectId + " option").each(function() {
							if ($(this).val() == $("#" + selectId).val()) {
								$(this).addClass("select-ul-item-selected");return false
							}
						});$("#ul-" + selectId + " option").css("display", "block");$(this).find("ul").css({
					...
					$("#" + selectId).val(selectValue);$("#input-" + selectId).val($(this).text());$("#ul-" + selectId).css("display", "none");opt.callback({
			...
			page : function(args) {
				...
				if (pageCount <= 1) {
					...
					html += '<li><a>下一页</a></li>';$("#" + pageId).append(html);init(pageId)
				} else {
				...
				function activePage(pageId, element) {
					element.addClass("active").siblings().removeClass("active");return $("#" + pageId).children("li.active").text()
				}
				function newPages(pageId, type, i) {
					...
					if (pageCount > 5 || pageCount < 3) {
						$("#" + pageId).empty();$("#" + pageId).append(html);init(pageId)
					}
				}
			},
			date : function(args) {
				...
				dateHtml += '</div>';
				dateHtml += '</div>';
				dateHtml += '</div>';$(document.body).append(dateHtml);init();close(true);$("#" + dateId).bind("click", function() {
					if (monthNum == 1) {
					...
					if (monthNum == 1) {
						...
						if (isTime || isTime == "true") {
							document.getElementById("hide-HMS-" + dateId).value = "";$("#date-hour-" + dateId + " li").removeClass("current");$("#date-minute-" + dateId + " li").removeClass("current");$("#date-second-" + dateId + " li").removeClass("current")
						}
					} else {
				...
				function init() {
					var exDate = this;
					$("#date-list-" + dateId).empty();var endDate = "";
					if (opt.endDate == "") {
						if (opt.date == "") {
							endDate = new Date()
						} else {
					...
					if (monthNum == 1 && (isTime || isTime == "true")) {
						...
						timeHtml += '</tr>';
						timeHtml += '</tbody>';
						timeHtml += '</table>';$("#date-list-" + dateId).append(timeHtml)
					} else {
						for (var i = 0; i < monthNum; i++) {
							var td = "";
							td = fillDate(endDate.getFullYear(), endDate.getMonth(), i);
					...
					this.calendarStartDate = new Date(endDate.getFullYear(), endDate.getMonth() + 1, 1);$("#last-year-" + dateId).bind("click", function() {
						exDate.calendarEndDate.setFullYear(exDate.calendarEndDate.getFullYear() - 1, exDate.calendarEndDate.getMonth(), 1);
						opt.endDate = date2ymd(exDate.calendarEndDate).join("-");init();return
						...
						$(this).addClass("current").siblings().removeClass("current");setHMStime();return
				...
				function removeCSS() {
					...
					for (var d = new Date(bDate); d.getTime() <= ced.getTime(); d.setDate(d.getDate() + 1)) {
						choice = "choice-style";$("#" + dateId + "_date_" + date2ymd(d).join("-")).removeClass(choice);$("#" + dateId + "_date_" + date2ymd(d).join("-")).removeClass("started").removeClass("ended").removeClass("selected")
					}
				}
				function addCSS() {
					if (monthNum == 1) {
						...
						if (isTime || isTime == "true") {
							...
							$("#date-hour-" + dateId + " li").each(function() {
								if ($(this).text() == arr[0]) {
									$(this).addClass("current").siblings().removeClass("current");return
								}
					...
					for (var d = new Date(startDate); d.getTime() <= endDate.getTime(); d.setDate(d.getDate() + 1)) {
						choice = "choice-style";$("#" + dateId + "_date_" + date2ymd(d).join("-")).removeClass("started").removeClass("ended").removeClass("selected");$("#" + dateId + "_date_" + date2ymd(d).join("-")).removeClass(choice);$("#" + dateId + "_date_" + date2ymd(d).join("-")).attr("class", choice)
					}
					$("#" + dateId + "_date_" + date2ymd(new Date(startDate)).join("-")).removeClass().addClass("started");$("#" + dateId + "_date_" + date2ymd(new Date(endDate)).join("-")).removeClass().addClass("ended")
				}
				function selectDate(ymd) {
					isClear = false;var ymdFormat = formatDate(ymd);
					if (this.dateInput == ("hide-date-" + dateId)) {
						removeCSS();$("#" + dateId + "_date_" + ymd).attr("class", "selected");$("#" + this.dateInput).val(ymdFormat);return
					}
					if ($("#" + this.dateInput).val() == "") {
						if (this.dateInput == ("hide-start-" + dateId)) {
							removeCSS();$("#" + dateId + "_date_" + ymd).attr("class", "selected");$("#" + this.dateInput).val(ymdFormat);
							this.dateInput = "hide-end-" + dateId
						} else if (this.dateInput == ("hide-end-" + dateId)) {
							if ($("#hide-start-" + dateId).val() == "") {
								this.dateInput = "hide-start-" + dateId;selectDate(ymd);return
							}
							...
							if (hideEndDate < hideStartDate) {
								...
								$("#hide-start-" + dateId).val($("#hide-end-" + dateId).val());$("#hide-end-" + dateId).val(tmp)
							}
							removeCSS();addCSS()
						}
					} else {
				...
				function show() {
					...
					$("#date-box-" + dateId).css("display", "block");$("#date-box-" + dateId).css("left", left + "px");$("#date-box-" + dateId).css("top", pos.top + $("#" + dateId).height() + 4 + "px");
					if (monthNum == 1) {
						this.dateInput = "hide-date-" + dateId;
						if (isTime || isTime == "true") {
							var diffH = $("#date-hour-" + dateId + " li.current").position().top;
							if (diffH > 220) {
								diffH = diffH - 83 + "px";$("#date-hour-" + dateId).animate({
									"scrollTop" : diffH
								}, 500)
							}
							var diffM = $("#date-minute-" + dateId + " li.current").position().top;
							if (diffM > 220) {
								diffM = diffM - 83 + "px";$("#date-minute-" + dateId).animate({
									"scrollTop" : diffM
								}, 500)
							}
							var diffS = $("#date-second-" + dateId + " li.current").position().top;
							if (diffS > 220) {
								diffS = diffS - 83 + "px";$("#date-second-" + dateId).animate({
									"scrollTop" : diffS
								}, 500)
							}
						}
				...
				function fillDate(year, month, index) {
					...
					dateBegin.setDate(1 - w);var lastDayOfMonth = new Date(year, month + 1, 0);
					var dateEnd = new Date(year, month + 1, 0);
					w = dateEnd.getDay();dateEnd.setDate(dateEnd.getDate() + 6 - w);var today = new Date();
					var table = document.createElement("table");
					if (monthNum == 1 && (isTime || isTime == "true")) {
						$(table).css("position", "relative")
					}
					cap = document.createElement("caption");$(cap).append(year + "年" + (month + 1) + "月");$(table).append(cap);
					thead = document.createElement("thead");
					tr = document.createElement("tr");var days = [ "日", "一", "二", "三", "四", "五", "六" ];
					for (var i = 0; i < 7; i++) {
						th = document.createElement("th");$(th).append(days[i]);$(tr).append(th)
					}
					...
					if (index == 0) {
						if (monthNum == 1 && (isTime || isTime == "true")) {
							$(td).append('<a href="javascript:void(0);" id="next-month-' + dateId + '"><span class="icon-chevron-right" style="position: absolute;top: 10px;right: 22px;color: #555;font-size: 14px;"></span></a>');$(td).append('<a href="javascript:void(0);" id="next-year-' + dateId + '"><span class="icon-step-forward" style="position: absolute;top: 8px;right: 0;color: #555;font-size: 16px;"></span></a>')
						} else {
							$(td).append('<a href="javascript:void(0);" id="next-month-' + dateId + '"><span class="icon-chevron-right" style="position: absolute;top: 10px;right: 30px;color: #555;font-size: 14px;"></span></a>');$(td).append('<a href="javascript:void(0);" id="next-year-' + dateId + '"><span class="icon-step-forward" style="position: absolute;top: 8px;right: 10px;color: #555;font-size: 16px;"></span></a>')
						}
					}
					if ((index + 1) == monthNum) {
						if (monthNum == 1 && (isTime || isTime == "true")) {
							$(td).append('<a href="javascript:void(0);" id="last-year-' + dateId + '"><span class="icon-step-backward" style="position: absolute;top: 8px;left: 0;color: #555;font-size: 16px;"></span></a>');$(td).append('<a href="javascript:void(0);" id="last-month-' + dateId + '"><span class="icon-chevron-left" style="position: absolute;top: 10px;left: 22px;color: #555;font-size: 14px;"></span></a>')
						} else {
							$(td).append('<a href="javascript:void(0);" id="last-year-' + dateId + '"><span class="icon-step-backward" style="position: absolute;top: 8px;left: 10px;color: #555;font-size: 16px;"></span></a>');$(td).append('<a href="javascript:void(0);" id="last-month-' + dateId + '"><span class="icon-chevron-left" style="position: absolute;top: 10px;left: 30px;color: #555;font-size: 14px;"></span></a>')
						}
					}
					$(td).attr("colSpan", 7);$(td).css("text-align", "center");$(tr).append(td);var tbody = document.createElement("tbody");
					$(tbody).append(tr);var tdClass = "";
					var deviation = 0;
					var ymd = "";
					for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
						...
						if (deviation == 0) {
							ymd = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate();$(td).attr("id", dateId + "_date_" + ymd);(function(ymd) {
								$(td).bind("click", ymd, function() {
									selectDate(ymd);return
								})
							})(ymd)
						}
						$(td).addClass("riqi");$(tr).append(td);
						if (d.getDay() == 6) {
							$(tbody).append(tr)
						}
					}
			...
			time : function(args) {
				...
				timeHtml += '</div>';
				timeHtml += '</div>';
				timeHtml += '</div>';$(document.body).append(timeHtml);addCSS();show();close(true);$("#" + timeId).bind("click", function() {
					initHMS = document.getElementById("hide-HMS-" + timeId).value;addCSS();show();return
				});$("#date-ok-" + timeId).bind("click", function() {
					close(true);opt.callback({
						"time" : getHMStime()
					});return
				});$("#date-cancel-" + timeId).bind("click", function() {
					document.getElementById("hide-HMS-" + timeId).value = initHMS;close();
					isClear = false;return
				});$("#date-clear-" + timeId).bind("click", function() {
					document.getElementById("hide-HMS-" + timeId).value = "";$("#date-hour-" + timeId + " li").removeClass("current");$("#date-minute-" + timeId + " li").removeClass("current");$("#date-second-" + timeId + " li").removeClass("current");
					isClear = true;return
				...
				function show() {
					...
					var left = pos.left;
					$("#date-box-" + timeId).css("display", "block");$("#date-box-" + timeId).css("left", left + "px");$("#date-box-" + timeId).css("top", pos.top + $("#" + timeId).height() + 4 + "px");
					if ($("#date-hour-" + timeId + " li.current").length > 0) {
						var diffH = $("#date-hour-" + timeId + " li.current").position().top;
						if (diffH > 220) {
							diffH = diffH - 83 + "px";$("#date-hour-" + timeId).animate({
								"scrollTop" : diffH
							}, 500)
						}
					}
					if ($("#date-minute-" + timeId + " li.current").length > 0) {
						...
						if (diffM > 220) {
							diffM = diffM - 83 + "px";$("#date-minute-" + timeId).animate({
								"scrollTop" : diffM
							}, 500)
						}
					}
					if ($("#date-second-" + timeId + " li.current").length > 0) {
						...
						if (diffS > 220) {
							diffS = diffS - 83 + "px";$("#date-second-" + timeId).animate({
								"scrollTop" : diffS
							}, 500)
						}
					}
				...
				if (type == "image") {
					$("#" + inputId).change(function() {
						...
						if (dataType == "base64") {
							...
							data.append(param, imageFile);info.optTip({
								content : "文件上传中，请稍候...",
								type : "submit"
							});$.ajax({
								url : url,
					...
					if (imgUrl != "" && imgUrl != null) {
						...
						for (var i = 0; i < imgUrl.length; i++) {
							var UUID = info.generateID();
							uuidArr.push(UUID);var imgHtml = '<div id=' + UUID + ' class="upload-images-thumbnail" style="width:' + opt.width + 'px;height:' + opt.height + 'px;">';
							imgHtml += '<img src="' + imgUrl[i] + '" width="100%" />';
							imgHtml += '<div class="img-trash"><span class="icon-trash-o"></span></div>';
							imgHtml += '</div>';$("#" + containerId).append(imgHtml)
						}
							...
							$.each($("#" + inputId)[0].files, function(i) {
								...
								if ((nowNum + i) <= (parseInt(maxNum) - 1)) {
									...
									reader.onload = function(event) {
										...
										imgHtml += '</div>';$("#" + containerId).append(imgHtml);imgBindEvent(uuidArr);count++;
										if (count == fileLength) {
					...
					$("#" + inputId).change(function() {
						...
						var data = new FormData();
						data.append(param, imageFile);$.ajax({
							url : url,
							type : "post",
							data : data,
							dataType : "json",
				...
				if (imageFile) {
					reader.onload = function(event) {
						...
						info.image.onload = function() {
							$("#" + opt.moveBox).hide();info.setBackgroundImage();info.headPreview();$("#" + info.imgDivId).bind("mousewheel DOMMouseScroll", info.imageZoom);$("#" + info.imgDivId).bind("mousedown", info.mouseDown);$("#" + info.imgDivId).bind("mousemove", info.imageDrag);$(window).bind("mouseup", info.mouseUp)
						}
					}
				}
				reader.readAsDataURL(imageFile)
			...
			enlarge : function() {
				this.ratio = this.ratio * 1.1;info.setBackgroundImage()
			},
			narrow : function() {
				this.ratio = this.ratio * 0.9;info.setBackgroundImage()
			},
			...
			imageZoom : function(event) {
				...
				info.setBackgroundImage();info.headPreview()
			},
			...
			mouseUp : function(event) {
				event.stopImmediatePropagation();
				info.dragAble = false;info.headPreview()
			},
			headPreview : function() {
				...
				$(".avatar180").html('');$(".avatar50").html('');$(".avatar30").html('');$(".avatar180").append('<img src="' + avatarPreviewImageSrc + '" align="absmiddle" style="width:180px;border-radius:180px;">');$(".avatar50").append('<img src="' + avatarPreviewImageSrc + '" align="absmiddle" style="width:50px;border-radius:50px;">');$(".avatar30").append('<img src="' + avatarPreviewImageSrc + '" align="absmiddle" style="width:30px;border-radius:30px;">');$("#" + info.dataUrl).val(avatarPreviewImageSrc)
			},
			edit : function(args) {
				var opt = defaults(args);
				var editId = opt.id;
				...
				editHtml += '<option value="18">18</option>';
				...
				editHtml += '</div>';
				editHtml += '</div>';$("#" + editId).append(editHtml);info.tip();$("#" + editId + " .edit-body-container").bind("mouseup keyup", function() {
					saveCurRange();opt.callback({
						"html" : $("#" + editId + " .edit-body-container").html().replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "").replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, ""),
						"text" : $("#" + editId + " .edit-body-container").text().replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "").replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "")
					})
				});
				var image = opt.image;
				if (image == null || image.dataType == "base64") {
					info.upload({
						...
						callback : function(rtn) {
							restoreSelection();execCommand("insertimage", false, rtn)
						}
					})
				} else {
					info.upload({
				...
				$("#" + editId + " .edit-btn-bold").bind("click", function() {
					restoreSelection();execCommand("bold", false, null);
					if (isBold) {
						isBold = false;$(this).removeClass("edit-active")
					} else {
						isBold = true;$(this).addClass("edit-active")
					}
					return false
				});
				var isItalic = false;
				$("#" + editId + " .edit-btn-italic").bind("click", function() {
					restoreSelection();execCommand("italic", false, null);
					if (isItalic) {
						isItalic = false;$(this).removeClass("edit-active")
					} else {
						isItalic = true;$(this).addClass("edit-active")
					}
					return false
				});$("#" + editId + " .edui-btn-justifyleft").bind("click", function() {
					execCommand("justifyleft", false, null);return false
				});$("#" + editId + " .edui-btn-justifycenter").bind("click", function() {
					execCommand("justifycenter", false, null);return false
				});$("#" + editId + " .edui-btn-justifyright").bind("click", function() {
					execCommand("justifyright", false, null);return false
				});$("#javaex-edit-family").bind("change", function() {
					execCommand("fontName", false, $("#javaex-edit-family").val())
				});$("#javaex-edit-size").bind("change", function() {
				...
				functionHtml += '$(".edit-body-container").append(\'<pre><code class="\'+codeStyle+\'">\'+code+\'</code></pre><p><br /></p>\');';
				functionHtml += '}';$(document.body).append('<script>' + functionHtml + '</script>');$("#" + editId + " .edui-btn-code").bind("click", function() {
					var html = '<textarea id="javaex-code" class="desc" style="height:200px;overflow: inherit;"></textarea>';
					html += '<ul class="equal-5" style="margin-top:10px;">';
					html += '<li><input type="radio" class="fill" name="code" value="html" checked/>html</li>';
					html += '<li><input type="radio" class="fill" name="code" value="style"/>css</li>';
					html += '<li><input type="radio" class="fill" name="code" value="javascript"/>js</li>';
					html += '<li><input type="radio" class="fill" name="code" value="xml"/>xml</li>';
					html += '<li><input type="radio" class="fill" name="code" value="java"/>java</li>';
					html += '<span class="clearfix"></span>';
					html += '</ul>';info.alert({
						title : "请输入要插入的代码",
						content : html,
				...
				function restoreSelection() {
					if (curRange) {
						var selection = null;
						selection = window.document.getSelection();selection.removeAllRanges();selection.addRange(curRange)
					}
				}
				function execCommand(command, mode, data) {
					document.execCommand(command, mode, data);opt.callback({
						"html" : $("#" + editId + " .edit-body-container").html().replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "").replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, ""),
						"text" : $("#" + editId + " .edit-body-container").text().replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "").replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "")
					})
				}
